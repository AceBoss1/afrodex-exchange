"use client"; // Marks the component as a Client Component, essential for using hooks like useState
...
// --- CONFIGURATION CONSTANTS (MOVED OUTSIDE OF APP COMPONENT) ---
// FIX: Placing constants here ensures they are available during Next.js server-side prerendering.
...
const PRIMARY_BG_CLASS = 'bg-amber-500'; // Define this one explicitly for use in TradeForm
                ...
                {['Limit', 'Market', 'Stop Limit'].map(type => (
                    <button
                        ...
                        className={`py-1 px-3 rounded-full transition duration-150 ${
                            tradeType === type
                                ? `${PRIMARY_BG_CLASS} text-gray-900 font-semibold` // <-- FIX: PRIMARY_BG_CLASS is now defined outside
                                : 'bg-gray-800 text-gray-400 hover:bg-gray-700'
                        }`}
                    >
                        {type}
    ...
    if (activeSection !== 'trade') {
        return (
            <div className="min-h-screen bg-gray-100 font-sans flex antialiased">
                <Sidebar isOpen={isSidebarOpen} setIsOpen={setIsOpen} setActiveSection={setActiveSection} />
                <div className="flex-1 flex flex-col overflow-hidden">
...
// FIX: Removing unused Firebase imports to resolve 'Module not found' warnings/errors.
// If Firebase is needed later, it must be installed via npm and used with necessary configuration.
